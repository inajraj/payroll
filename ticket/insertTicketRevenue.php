...
if ($ticketid > 0)
    //update
    $sql = "UPDATE ticketmaster SET Driver1 =  '" . $results['driver1'] . "', Driver2 = '" . $results['driver2'] .  "', Cleaner = '" . 
    $results['cleaner'] . "' WHERE ID = " . $ticketid  ;
else
    //insert
    $sql = "INSERT INTO ticketmaster(`BusNumber`, `Date`, `Driver1`, `Driver2`, `Cleaner`) 
...
//echo $sql;
 ...
 if ($ticketid==0)
 {
    ...
    $sql = "select ID from ticketmaster WHERE Date = '" . $results['inputDate'] . "' and BusNumber = '" .  $results['bn'] . "'";
    $result = $conn->query($sql);
